...
// map original names to array index
const FOLDER_TO_INDEX = {
  'a': 0,
  'b': 1,
  'c': 2,
  'd': 3,
...
const loadFolder = async (relPath) => {
  ...
  for (const file of files) {
    ...
    } else if (typeof FOLDER_TO_INDEX[file.name] !== 'undefined') {
      // A known folder. Traverse.
      folders[FOLDER_TO_INDEX[file.name]] = await loadFolder(file.path);
    }
  }
  return folders;
};
...
var getTimezone = function (lat, lon) {
  if (!tzQuads) {
    throw new Error('Not loaded. Forgot to call init()?');
  }
  lat = parseFloat(lat)
  lon = parseFloat(lon)
  ...
  while (true) {
    ...
    if (!curTzData) {
      ...
      var geoJson = quadPos.split('').reduce((el, name) => el && el[name.charCodeAt(0) - 97], tzQuads)
...
module.exports.loaded = loadTree();
